...
The functionality of any Java application or library is structured as a call graph of independent methods. Simple methods may only refer to constant data or, in the case of instance methods, local instance state. However, most methods are parameterised, allowing callers to share data with methods they call via one or more call arguments. Arguments are always bound locally to a specific call and hence also locally to the the thread executing a given call hierarchy.

Most of the time method parameters are an effective and convenient way to communicate and share data. However, it does not always scale well in large applications built using independently developed and maintained components. For example, a web framework might include a server component, implemented in the [thread-per-request style](https://openjdk.org/jeps/444#The-thread-per-request-style), and a data access component, which handles persistence. The server may associate an incomping request with an identity which it needs to share with the data access component. However, it is normally the responsibility of the application request handler rather than the server to decide whether, for example, to open a database. In order to achieve the requried sharing sharing using method parameters the framework must pass the user identity as an argument to the request handler call. The handler code is then responsible for passing this argument on down the call chain to any point where it calls the relevant data access component method.

There are several reasons why this can become a problem. The obvious issue is that server methods are now complicated by the need to receive and correctly pass on data that is does not itself directly consume. When the call hierarchy and control flow in the server thread is large and complex this may require correctly wiring the parameter flow through many call chains. If the server thread avoids this wiring problem by storing the parameter as object state then it must ensure that this state is read by the correct thread and does not get overwritten or go stale before it is passed on the data access component. Either way, the resulting code is now more brittle than it needs to be: if the framework and data access component need to change the way they share data or the type and number of data values they share then the server thread may also need to be updated.
...
The correct Identity for each thread can be shared by `Server.serve` and `DBAccess.open` by passing it as an argument through the chain of calls to the `Application` methods. However, it would be cleaner, less prone to error and easier to maintain if there were some way for the two components to _directly_ share the value belonging to the current thread.
...
thread-local variable to share a `Identity`. The server component
