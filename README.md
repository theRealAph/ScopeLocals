...
Like a thread-local variable, a scoped value has multiple incarnations, one per thread. The particular incarnation that is used depends on which thread calls its methods. Unlike a thread-local variable, a scoped value provides highly constrained mutability. By default  a scoped value is unset in any new thread. It can only be set for the duration of a specific call and automatically reverts to unset once the call completes. Called methods are allowed temporarily to reset the scoped value, but only for the duration of a nested call. Once that nested call completes the scoped value is automatically reverted to its setting prior to the nested call.
